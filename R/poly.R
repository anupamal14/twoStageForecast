...
twoStage.poly <- function(d, freq, nrDays){
  ...
  meanBy <- as.vector(t(matrix(rep(c(1:(length(d)/freq)),freq),
                               nrow = length(d)/freq)))
  ...
  seas <- list()
  seas$mult <- rep(cpoly_season_mult, length(d)/freq/nrDays)
  seas$add <- rep(cpoly_season_add, length(d)/freq/nrDays)
  return(seas)
}
