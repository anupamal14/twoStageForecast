...
firstStage <- function(ylow, n, p, seas_periods, regMat = NULL,
  ...
  for (d in 0:2)
    for (D in 0:2) {
      fit <- auto.arima(ylow[1:n], d = d, D = D, max.p = 6, max.q = 6, max.P = 6, max.Q = 6,
                        ...
                        seasonal = TRUE)
      psi <- twoStage.penalizedLikelihood(mean(fit$residuals^2), length(fit$residuals),
                                          (length(fit$coef) + 1))
      if (psi < minPsi) {
        fit_ARIMA <- fit
        minPsi <- psi
  ...
  if (!is.null(regMat)){
    ...
    if(plotFlag){
      ...
      # Stage 1 plot
      plot(1:(n+p),ylow[1:(n+p)] , type = 'l', lwd = 3, col = "blue",
           xlab = "Time", ylab = "Time Series")
      lines(1:n, ylow[1:n], lwd = 3)
      lines(c(fit_TBATS$fitted.values[1:n],yhat_TBATS), col = 'cyan')
      lines(c(xhat_ARIMA[1:n],yhat_ARIMA), col = 'orange')

      legend("topright",c("Actual Model Period","Actual Holdout",
                             "TBATS", "ARIMA"),
